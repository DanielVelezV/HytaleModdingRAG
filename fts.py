...
def hybrid_search(
    ...
    # The identifier gate exists so keyword noise doesn't dilute good dense hits.
    # With no dense results (lite mode) keyword search is all we have, so always run it.
    has_identifier = any(_IDENT_RE.search(t) for t in query.split())
    if has_identifier or not dense_results:
