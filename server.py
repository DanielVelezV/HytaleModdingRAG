...
_NO_DATA_MSG = (
    ...
    "  pip install -e .\n"
    "  hytale-rag\n\n"
    "The standard index is a ~36 MB download and needs nothing else."
)


def _require_data() -> str | None:
    """Data is present if either tier's store exists."""
    lite = (DATA_DIR / "fts.sqlite").exists() and (DATA_DIR / "meta.sqlite").exists()
    full = CHROMADB_DIR.exists() and any(CHROMADB_DIR.iterdir())
    return None if (lite or full) else _NO_DATA_MSG


# Building and publishing the index is a maintainer operation: it needs the
# licensed HytaleServer.jar, and consumers get the result via GitHub Releases.
# These tools are registered only when HYTALE_RAG_ADMIN=1, so they don't appear
# in the tool list for anyone running the published server. The same operations
# live in admin.py as a CLI.
ADMIN_MODE = os.environ.get("HYTALE_RAG_ADMIN") == "1"


def admin_tool():
    def deco(fn):
        return mcp.tool()(fn) if ADMIN_MODE else fn
    return deco
...
The index is pre-built and distributed via GitHub Releases; it is not built or
modified from here. If the data looks out of date, tell the user to run
`hytale-rag update` rather than attempting to re-index.
...
def _classname_token_boost(query: str, api_results: list[dict]) -> list[dict]:
    ...
    from store import get_collection
    from descriptions import CLASS_DESCRIPTIONS as _CLASS_DESCRIPTIONS
...
def get_class_info(class_name: str) -> str:
    ...
    if err:
        return err
    from store import get_collection, API_COLLECTION
    ...
    if not results["ids"]:
        from store import _get_embedding_fn
...
def find_usages(name: str, limit: int = 10) -> str:
    ...
    from store import search as idx_search
